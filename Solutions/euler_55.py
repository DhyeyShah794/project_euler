...
theoretical nature of these numbers, and for the purpose of this problem, we shall assume that a number is Lychrel until
proven otherwise. In addition you are given that for every number below ten-thousand, it will either
...
def lychrel_check(num: int) -> bool:
    for i in range(51):
...
def lychrel_count(upper_limit: int) -> int:
    values = [i for i in range(upper_limit) if lychrel_check(i)]
    return len(values)
